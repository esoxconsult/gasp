...
\begin{abstract}
I present GASP (Gaia Asteroid Spectral Pipeline), an open-source
...
I validate the NUV correction against the Eight Color Asteroid
Survey (ECAS; Zellner et al. 1985), finding Pearson
$r = -0.848$ ($n=83$, $p \approx 0$), confirming consistency
with ground-based photometry. The reproducible catalog and pipeline are
...
The reproducible processing pipeline and the resulting catalog are archived on Zenodo \citep{scheibenpflug_2026_gasp}.
...
The catalog illustrates the SDSS $a^*$ color index
\citep[a C/S-complex separator;][]{Ivezic2002} plotted
against the NUV slope $s_1$, colored by NEOWISE albedo.
C-complex asteroids ($a^* < -0.10$) show lower albedo (mean $p_V \approx 0.06$) and more negative
$s_1$ values, consistent with hydrated, UV-absorbing
primitive material. S-complex asteroids ($a^* > 0.10$) show
higher albedo (mean $p_V \approx 0.22$) and flatter NUV
slopes, consistent with anhydrous silicate compositions.
The same figure is in the Zenodo exploration notebook
(\href{https://doi.org/10.5281/zenodo.19366681}
{10.5281/zenodo.19366681}).
...
%% Bibliography from docs/references.bib (run from docs/: pdflatex ms; bibtex ms; pdflatex ms; pdflatex ms)
